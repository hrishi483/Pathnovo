"""Load .dwg/.dxf drawings and extract raw CAD text/geometry records."""
...
def _libredwg_convert(path: Path, output_dir: Path) -> Path:
    exe = shutil.which("dwg2dxf")
    if exe is None:
        raise DWGConversionError("LibreDWG dwg2dxf was not found on PATH")
    out_file = output_dir / (path.stem + ".dxf")
    try:
        subprocess.run(
            [exe, "-o", str(out_file), str(path)],
            check=True,
            capture_output=True,
            text=True,
        )
    except subprocess.CalledProcessError as exc:
        raise DWGConversionError(f"LibreDWG dwg2dxf failed: {exc.stderr}") from exc
...
def load_document(path: str | Path) -> Drawing:
    """Load a .dwg or .dxf file into an ezdxf Drawing."""
    path = Path(path)
    if not path.is_file():
    ...
    if shutil.which("dwg2dxf") is not None:
        with tempfile.TemporaryDirectory(prefix="dwg2dxf_") as temporary:
            dxf_path = _libredwg_convert(path, Path(temporary))
            return ezdxf.readfile(str(dxf_path))

    raise DWGConversionError(
        "Neither the ODA File Converter nor LibreDWG's `dwg2dxf` was found. "
        "Install one to read .dwg files. (.dxf files don't need either.)"
    )
...
def _extract_text(e, layout_name: str, source_block: str | None) -> dict | None:
    ...
    if t == "MTEXT":
        ...
        rotation = (
            e.get_rotation() if hasattr(e, "get_rotation") else getattr(e.dxf, "rotation", 0.0)
        )
        return {
            "handle": handle,
            "entity_type": "MTEXT",
            ...
            "text": text,
            "insert": _point3(e.dxf.insert),
            "height": getattr(e.dxf, "char_height", None),
            "rotation": rotation,
            "style": getattr(e.dxf, "style", None),
            "color": color,
            "source_block": source_block,
...
def _extract_geometry(e, layout_name: str, source_block: str | None) -> dict | None:
    ...
    if t == "ELLIPSE":
        return make(
            {
                "center": _point3(e.dxf.center),
                "major_axis": _point3(e.dxf.major_axis),
                "ratio": e.dxf.ratio,
                "start_param": getattr(e.dxf, "start_param", 0.0),
    ...
    if t == "SPLINE":
        return make(
            {
                "control_points": [_point3(p) for p in e.control_points],
    ...
    if t == "HATCH":
        ...
        for path in e.paths:
            if hasattr(path, "vertices"):
                paths.append([(float(v[0]), float(v[1])) for v in path.vertices])
            else:
                paths.append([])
        return make(
            {
    ...
    if t in ("XLINE", "RAY"):
        return make(
            {
                "point": _point3(e.dxf.start),
                "direction": (
                    _point3(e.dxf.unit_vector) if e.dxf.hasattr("unit_vector") else None
                ),
            }
        )
    if t == "MESH":
...
def extract(
    ...
    layouts: "modelspace" (default), "all", or comma-separated layout names.
    """
    ...
    if layouts == "all":
        ...
        names = [name.strip() for name in layouts.split(",")]
        layout_iter = [(name, doc.layouts.get(name)) for name in names]
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 1:
        print("Usage: python -m src.ingest.cad_extract <path-to-drawing.dwg-or-.dxf>")
        return 1
    ...
    print(
        f"Extracted {len(result['text'])} text entities and "
        f"{len(result['geometry'])} geometry entities from {path}"
    )
